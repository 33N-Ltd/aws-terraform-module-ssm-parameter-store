...
output "names" {
  value       = join("", local.name_list)
  description = "A list of all of the parameter names"
}

output "values" {
  description = "A list of all of the parameter values"
  value       = join("", local.value_list)
}

output "map" {
  description = "A map of the names and values created"
  value       = join("", zipmap(local.name_list, local.value_list))
}

output "map_names" {
  description = "A list of all of the parameter names"
  value       =  join("", local.name_list_map)
}

output "map_values" {
  value       = join("", local.value_list_map)
  description = "A list of all of the parameter names"
}

output "kv" {
  description = "A map of the names and values created"
  value       = join("", zipmap(coalescelist(keys(var.read_parameter_map), local.name_list),
